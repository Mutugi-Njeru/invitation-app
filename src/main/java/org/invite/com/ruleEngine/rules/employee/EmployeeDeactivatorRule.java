...
@ApplicationScoped
public class EmployeeDeactivatorRule implements ServiceRule {
    @Inject
    EmployeeService employeeService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.DEACTIVATE_EMPLOYEE.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=employeeService.deactivateEmployee(request);
