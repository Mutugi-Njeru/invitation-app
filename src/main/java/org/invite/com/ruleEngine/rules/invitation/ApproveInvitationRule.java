...
@ApplicationScoped
public class ApproveInvitationRule implements ServiceRule {
    @Inject
    ...
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.APPROVE_INVITATION.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=invitationService.approveInvitation(request);
        return (response.isSuccess())
                ? Util.buildResponse(Response.Status.OK.getStatusCode(), ResultIds.APPROVE_SUCCESSFUL.name(), response.message())
