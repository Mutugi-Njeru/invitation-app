...
@ApplicationScoped
public class GetAllStructuresRule implements ServiceRule {
    @Inject
    StructureService structureService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.GET_ALL_STRUCTURES.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=structureService.getAllStructures(request);
