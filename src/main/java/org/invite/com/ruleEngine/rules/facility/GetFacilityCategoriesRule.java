...
@ApplicationScoped
public class GetFacilityCategoriesRule implements ServiceRule {
    @Inject
    FacilityService facilityService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.GET_FACILITY_CATEGORIES.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=facilityService.getFacilityCategories(request);
