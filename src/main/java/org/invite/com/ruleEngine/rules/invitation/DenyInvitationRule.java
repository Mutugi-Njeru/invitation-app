...
@ApplicationScoped
public class DenyInvitationRule implements ServiceRule {
    @Inject
    InvitationService invitationService;

    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.REJECT_INVITATION.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=invitationService.rejectInvitation(request);
        return (response.isSuccess())
                ? Util.buildResponse(Response.Status.OK.getStatusCode(), ResultIds.DENY_SUCCESSFUL.name(), response.message())
