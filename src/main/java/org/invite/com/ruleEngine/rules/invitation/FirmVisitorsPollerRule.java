...
@ApplicationScoped
public class FirmVisitorsPollerRule implements ServiceRule {
    @Inject
    InvitationService invitationService;

    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.GET_FIRM_VISITORS.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=invitationService.getAllVisitors(request);
