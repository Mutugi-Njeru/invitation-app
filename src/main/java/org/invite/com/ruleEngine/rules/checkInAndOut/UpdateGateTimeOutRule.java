...
@ApplicationScoped
public class UpdateGateTimeOutRule implements ServiceRule {
    @Inject
    CheckInGateService checkInGateService;

    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.UPDATE_GATE_TIME_OUT.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=checkInGateService.updateGateTimeOut(request);
