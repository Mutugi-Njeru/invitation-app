...
@ApplicationScoped
public class UpdateTimeInFirmRule implements ServiceRule {
    @Inject
    InvitationService invitationService;

    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.UPDATE_TIME_IN.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=invitationService.updateVisitorTimeInTheFirm(request);
