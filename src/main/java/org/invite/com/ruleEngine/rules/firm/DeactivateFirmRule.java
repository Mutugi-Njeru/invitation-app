...
@ApplicationScoped
public class DeactivateFirmRule implements ServiceRule {
    @Inject
    FirmService firmService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.DEACTIVATE_FIRM.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=firmService.deactivateFirm(request);
