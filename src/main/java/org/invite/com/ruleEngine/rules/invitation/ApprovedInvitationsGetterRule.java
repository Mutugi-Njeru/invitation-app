...
@ApplicationScoped
public class ApprovedInvitationsGetterRule implements ServiceRule {
    @Inject
    InvitationService invitationService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.GET_APPROVED_INVITATIONS.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=invitationService.getApprovedInvitations(request);
