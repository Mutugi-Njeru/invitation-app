...
@ApplicationScoped
public class GetFirmEmployeesRule implements ServiceRule {
    @Inject
    EmployeeService employeeService;
    @Override
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.GET_FIRM_EMPLOYEES.name()));
    }

    @Override
    public JsonObject apply(Object input) {
        JsonObject request= Util.convertStringToJson(input.toString());
        ServiceResponder response=employeeService.getFirmEmployees(request);
